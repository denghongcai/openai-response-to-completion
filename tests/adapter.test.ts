import { OpenAIResponseAdapter } from "../src/index";
...
describe("OpenAIResponseAdapter", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new OpenAIResponseAdapter(mockOpenAIClient as OpenAI);
  });
  ...
  describe("Non-Streaming: chat.completions.create", () => {
    ...
    it("should handle n > 1 using parallel strategy", async () => {
      const mockResponse1 = {
        ...
        status: "completed", status_details: { type: "stop" },
  ...
  describe("Streaming: chat.completions.create({ stream: true })", () => {
    it("should yield ChatCompletionChunk objects for a text stream", async () => {
      async function* mockStream() {
        yield { type: "response.output_text.delta", item_id: "stream-123", delta: "Hello" };
        yield { type: "response.output_text.delta", item_id: "stream-123", delta: ", " };
        yield { type: "response.output_text.delta", item_id: "stream-123", delta: "world!" };
        yield {
    ...
    it("should yield ChatCompletionChunk objects for a tool call stream", async () => {
        async function* mockToolStream() {
            yield { type: "response.output_tool_calls.delta", item_id: "stream-tool-123", payload: { index: 0, id: "call_abc", type: "function", function: { name: "get_weather" } } };
            yield { type: "response.output_tool_calls.delta", item_id: "stream-tool-123", payload: { index: 0, function: { arguments: '{"loc' } } };
            yield { type: "response.output_tool_calls.delta", item_id: "stream-tool-123", payload: { index: 0, function: { arguments: 'ation":"Boston"}' } } };
            yield {
              type: "response.completed",
              response: {
        ...
        expect(chunks[1].choices[0].delta.tool_calls?.[0].function?.arguments).toBe('{"loc');
        expect(chunks[2].choices[0].delta.tool_calls?.[0].function?.arguments).toBe('ation":"Boston"}');
    ...
    it("should throw an error if n > 1 for streams", async () => {
        await expect(adapter.chat.completions.create({
            ...
            stream: true,
            n: 2,
        })).rejects.toThrow("Streaming with `n > 1` is not supported in a single request.");
    });
  });
});